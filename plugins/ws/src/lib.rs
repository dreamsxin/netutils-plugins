...
//
// 实现放在 lib 里，两个二进制（`netutils-ws` 与保留的旧别名 `netutils-websocket`）
// 都只是薄壳调用 [`run_cli`]。此前别名靠 `include!("../main.rs")` 复制整份源码，
// 会让单元测试被编译执行两遍，clippy 也重复检查同一文件。
...
/// 解析命令行并执行一次 WebSocket 诊断。两个二进制入口共用此函数。
